...
def preprocess_data():
    ...
    fundings_df = fundings_df.with_columns(
        pl.lit(2023).alias("election_year"),
        pl.col("'Tukilahteen lomakenumerot'")
        .str.strip_chars("'")
        .replace(funding_type_map)
        .alias("ftype"),
        pl.col("'Tukija'").str.strip_chars("'").alias("funder_organization"),
        pl.col("'Tukijan y-tunnus/yhdistysrekisterinumero jos on'")
        .str.strip_chars("'")
        .alias("funder_company_id"),
        ...
        pl.col("'Lainan takaisinmaksuaika/laina-aika'")
        .str.strip_chars("'")
        .alias("loan_schedule"),
        pl.col("'Tuen maara'")
        .str.strip_chars("'")
    ...
    fundings_df = fundings_df.join(mp_df, on=["first_name", "last_name"], how="inner")

    # Drop useless mp name columns
    fundings_df = fundings_df.drop(["first_name", "last_name"])

    # Reorder
    fundings_df = fundings_df.select(
        [fundings_df.columns[-1], *fundings_df.columns[:-1]]
    )
...
def import_data():
    ...
    with open(csv_path) as f:
        cursor.copy_expert(
            "COPY election_fundings(person_id, election_year, ftype, funder_organization, funder_company_id, funder_first_name, "
            "funder_last_name, loan_title, loan_schedule, amount) FROM stdin DELIMITERS ',' CSV HEADER QUOTE '\"';",
            f,
        )
