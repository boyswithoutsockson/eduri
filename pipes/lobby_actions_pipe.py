...
def preprocess_data():
    ...
    df = df.filter(
        pl.col("topics").is_not_null()
    )  # Drop lobbies that have not lobbied yet, only registered
    ...
    df = (
        df.explode("contactedTargets")
        .unnest("contactedTargets")
        .explode("contactMethods")
        .drop("activityNotificationTopicId")
    )

    df = df.with_columns(
        pl.coalesce([pl.col("companyId"), pl.col("otherCompanyId")]).alias("lobby_id")
    ).drop(["companyId", "otherCompanyId"])

    df = df.with_columns(
        pl.coalesce([pl.col("fi"), pl.col("contactTopicOther")]).alias("topic")
    ).drop(["contactTopicOther", "fi"])
    ...
    # Parsing actions
    persons_df = match_target_mp(
        df["contactedTargetId"]
    )  # Matching targets to persons in database (only mps for now)

    actions_df = df.join(
        persons_df, right_on="target_id", left_on="contactedTargetId", how="inner"
    )

    actions_df = actions_df.select(
        pl.col("lobby_id"),
