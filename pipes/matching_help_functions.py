...
def match_target_mp(target_ids):
    ...
    targets_df = pl.read_json(json_path).drop(["createdAt", "id", "fiId", "svId", "enId", "termId", "hash", "sv", "en"])
    ...
    targets_df = targets_df.with_columns(pl.col("name").str.split(by=" ").list.get(0).alias("target_first_name"),
    ...
    matches_df = joined_df.filter(pl.col("target_id").is_in(target_ids))    # Only pick the ones in the list of wanted targets
