...
from XML_parsing_help_functions import id_parse, date_parse, Nimeke_parse, AsiaSisaltoKuvaus_parse, Perustelu_parse, Saados_parse, Ponsi_parse, status_parse, Allekirjoittaja_parse
...
interpellation_signatures_csv = os.path.join("data", "preprocessed", "interpellation_signatures.csv")
...
def preprocess_data():
    ...
    for interpellation_xml_str in interpellation_df.get("XmlData", []):
        ...
        if interpellation is None:                                                              # Joskus oikean välikysymyksen lisäksi on tyhjä välikysymys samalla id:llä                         
            if len(interpellation_df.loc[interpellation_df['Eduskuntatunnus'] == eid]) > 1:     # Tarkistetaan että samalla id:llä löytyy toinenkin (oikea) välikysymys
                continue
            else:
                raise Exception
    ...
    pd.DataFrame(interpellation_records).to_csv(interpellations_csv, index=False, encoding="utf-8")
    pd.DataFrame(sgn_records).drop_duplicates().to_csv(interpellation_signatures_csv, index=False, encoding="utf-8")
