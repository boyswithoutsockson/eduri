...
def preprocess_data():
    ...
    lobbies_df = lobbies_df.with_columns(           
        ...
        .otherwise(pl.col("otherCompanyId"))        # others have their own id under "otherCompanyId"
        .alias("lobby_id"))
    ...
    lobbies_df = lobbies_df.sort("industry", nulls_last=True).group_by("lobby_id").first()
...
def import_data():
    ...
    with open(csv_path) as f:
        cursor.copy_expert("COPY lobbies(id, name, industry) FROM stdin DELIMITERS ',' CSV HEADER QUOTE '\"';", f)
